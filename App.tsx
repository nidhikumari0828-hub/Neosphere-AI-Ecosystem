...
import { Menu, LayoutGrid, Activity, Share2, Languages, Archive, Cpu, Key, Sparkles, AlertTriangle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Bypass API key check by default
    ...
    try {
      ...
      if (options?.videoGen) {
        ...
        result = await processUserRequest(
          ...
          {
            ...options,
            protocol: protocols.find(p => p.isActive)
          },
          customAgents
        );
      }
      ...
      const modelMsg: Message = {
        ...
        mediaType: result.mediaUrl ? (options?.videoGen ? 'video' : 'image') : undefined,
      ...
      setMessagesByAgent(prev => ({
        ...prev,
        [activeAgentId]: [...(prev[activeAgentId] || []), modelMsg]
      }));
    } catch (error: any) {
      ...
      const errorMsg: Message = {
        ...
        \nAttempting interface recalibration...`,
        timestamp: Date.now(),
        agentId: activeAgentId
      };
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      <Sidebar 
        ...
        onSelectAgent={(id) => { setActiveAgentId(id); setView('chat'); }} 
        ...
        <div className="flex-none p-4 flex justify-between items-center bg-slate-950/50 border-b border-slate-800/50 z-30">
          <div className="flex items-center space-x-2">
            ...
            <div className="hidden md:flex items-center space-x-1 px-3 py-1 bg-cyan-500/10 border border-cyan-500/20 rounded-full">
              ...
              <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest">System Load: 12%</span>
            </div>
          </div>

          <div className="flex bg-slate-900/80 p-1 rounded-2xl border border-slate-800 backdrop-blur-md">
            {[
              { id: 'chat', icon: Cpu, label: 'Core' },
              { id: 'network', icon: LayoutGrid, label: 'Nodes' },
              { id: 'stats', icon: Activity, label: 'Analytics' },
              { id: 'lab', icon: Languages, label: 'Lab' },
              { id: 'vault', icon: Archive, label: 'Vault' },
              { id: 'creator', icon: Sparkles, label: 'Forge' },
            ].map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${view === item.id ? 'bg-white text-black shadow-lg' : 'text-slate-500 hover:text-slate-200'}`}
                >
        ...
        <div className="flex-1 overflow-y-auto relative z-10">
          <div className="h-full p-6">
            ...
            {view === 'lab' && (
              <LinguisticLab 
                protocols={protocols} 
                onAddProtocol={(p) => setProtocols([...protocols, p])} 
                onDeleteProtocol={(id) => setProtocols(protocols.filter(p => p.id !== id))} 
                onToggleProtocol={(id) => setProtocols(protocols.map(p => p.id === id ? { ...p, isActive: !p.isActive } : { ...p, isActive: false }))} 
              />
            )}
